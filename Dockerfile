...
#
#   docker build -t asgs/2019stable .
#
# Run command:   (used by most, drops them directly to the ASGS Shell prompt)
#
#   docker pull asgs/2019stable    # get from dockerhub
#   docker run -it asgs/2019stable # create running container from pulled image
...
RUN wget https://github.com/jasonfleming/asgs/archive/2019stable.tar.gz
...
RUN perl -pi -e 's/Date::Format/Date::Format Date::Handler/g' /asgs-2019stable/cloud/general/init-perl-modules.sh

# directly runs asgs-brew.pl
RUN cd /asgs-2019stable && ./cloud/general/asgs-brew.pl --machinename vagrant --compiler gfortran --asgs-profile default

# run whenever user creates a running container of this image
# via `docker run -it asgs/2019stable`
ENTRYPOINT /root/bin/asgsh
